...
module CalendarHelper
  ...
  def calendar(options = {}, &block)
    ...
    if options[:previous_month_text] or options[:next_month_text]
      cal << %(<th class= "prevMonthName" colspan="2">#{options[:previous_month_text]}</th>)
      colspan=3
    else
      colspan=7
    end
    cal << %(<th colspan="#{colspan}" class="#{options[:month_name_class]}">#{Date::MONTHNAMES[options[:month]]}</th>)
    cal << %(<th class= "nextMonthName" colspan="2">#{options[:next_month_text]}</th>) if options[:next_month_text]
